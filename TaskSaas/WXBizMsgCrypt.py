...
class WXBizMsgCrypt(object):
    ...
    def DecryptMsg(self, sPostData, sMsgSignature, sTimeStamp, sNonce):
        ...
        if ret != 0:
            return ret, None
        sha1 = SHA1()
        print('self.echoStr>>',self.echoStr)
        ret,signature = sha1.getSHA1(self.token, sTimeStamp, sNonce, self.echoStr)
        ...
        pc = Prpcrypt(self.key)
        ret,xml_content = pc.decrypt(self.echoStr,self.appid)
        print('ret>> ',ret)
        print('xml_content>> ',xml_content)
        return ret,xml_content

    # def verifyUrl(self,token,msgSignature,timestamp,nonce,echoStr):
    #     sign= SHA1().getSHA1(token,timestamp,nonce,echoStr)
    #     if sign
