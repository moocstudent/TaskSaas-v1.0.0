...
class GloryModelForm(BootStrapForm, forms.ModelForm):
    ...
    class Meta:
        model = models.Glory
        fields = ['name', 'color', 'desc','pic_url','required_score']
        widgets = {
            'desc': forms.Textarea,
    ...
    def clean_name(self):
        ...
        exists = models.Glory.objects.filter(name=name).exists()
        if exists:
            raise ValidationError('荣誉名已存在！')
